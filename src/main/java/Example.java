...
@CommandLine.Command(sortOptions = false, name = "Example", mixinStandardHelpOptions = true, version = "Tp5_Hunter v1.0", synopsisHeading = "", customSynopsis = {
        "\n", "================================================================================", "工具名称：Tp5_Hunter v1.0", "开发者：Vulner-6", "Github地址：https://github.com/Vulner-6", "================================================================================", "命令语法：", "java -jar tool.jar <url来源> <利用方式> <是否添加本地代理地址>", "java -jar tool.jar <-option> [value]...[-option] [value]\n"
}, footer = {
        "\n用法示例————(真实使用时请以导出的jar包名为准)", "java -jar tool.jar -u http://www.example.com --poc", "java -jar tool.jar -r list.txt -p", "java -jar tool.jar -fofa email_address,api_key --poc", "java -jar tool.jar -s 127.0.0.1:1080 -r list.txt -p", "--------------------------------------------------------------------------------", "注意:", "1.表示目标范围的选项，只能选择一个，比如-u和-r就互相冲突，只能选其一。", "2.每次批量扫描后，生成的results.txt会被覆盖，如果需要，请备份。", "3.可能还存在未知的BUG，初次开发，请多包涵。", "--------------------------------------------------------------------------------"
})
public class Example implements Runnable
{
    ...
    public void run()
    {
        ...
        if (this.fofa != null && this.fofa.length > 0 && this.poc)
        {
            ...
            System.out.print("请输入你希望查询到的数据个数（当真实数据超过指定条数，只取指定条数返回；" + "当真实数据不足指定条数，就返回全部真实数据；" + "当真实数据为0，就什么都不返回）：");
            ...
            if (test > 0)
            {
                ...
                {
                    System.out.println("与fofa建立连接，向fofa发送查询请求时发生异常，请检查账号和api_key是否正确" + "程序退出（也可能是fofa服务器出现问题，导致无法查询）");
                }
                //遍历fofa查询返回的数组
